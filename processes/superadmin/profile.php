...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($new_name) || empty($new_email)) {
        ...
        if ($stmt->execute()) {
            ...
            // Log the activity
            logActivity($conn, $_SESSION['user_id'], 'Update Profile', "Updated profile information.");
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        // Fetch the current password from the database (for all roles)
        $sql = "SELECT password FROM tbl_users WHERE id = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($user && password_verify($current_password, $user['password'])) {
            ...
            // Update the password in the database (for all roles)
            ...
            if ($stmt->execute()) {
                ...
                // Log the activity
                logActivity($conn, $_SESSION['user_id'], 'Change Password', "Changed password.");
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_2fa'])) {
    ...
    if ($stmt->execute()) {
        $success_message = "Two-factor authentication " . ($two_factor_auth ? "enabled" : "disabled") . " successfully!";

        // Log the activity
        logActivity($conn, $_SESSION['user_id'], 'Toggle 2FA', ($two_factor_auth ? "Enabled" : "Disabled") . " two-factor authentication.");
    } else {
        $error_message = "Failed to update two-factor authentication. Please try again.";
    }
}
